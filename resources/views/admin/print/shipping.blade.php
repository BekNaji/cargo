...
@section('contetn')
    ...
    <div class="col-12 position-relative" >
        <div class="district-bg">
            {{$shipping->receiver->addresses->region->name_uz ?? ''}} <br>
            {{$shipping->receiver->addresses->district->name_uz ?? ''}} <br>
        </div>
        <div class="row">
            <div class="col-6">
            ...
            <div class="col-6 text-center mt-3 d-flex justify-content-center align-items-center">
                <h3 style="font-size: 40px; transform: scale(2.5);">INVOICE NO: <span class="text-danger">{{$shipping->number ?? ''}}</span></h3>
            </div>
                ...
                <table class="table-c-border w-100 print-c-table">
                    <thead>
                        <tr>
                            ...
                            <td colspan="3">{{$shipping->sender->name ?? ''}}</td>
                        </tr>
                        <tr>
                            <td colspan="2">Receiver</td>
                            <td colspan="3">{{$shipping->receiver->name ?? ''}}</td>
                        </tr>
                        <tr>
                            <td colspan="2">Receiver's Address</td>
                            <td colspan="3">
                                {{$shipping->receiver->addresses->region->name_uz ?? ''}},
                                {{$shipping->receiver->addresses->district->name_uz ?? ''}},
                                {{$shipping->receiver->addresses->open_address}}</td>
                        </tr>

                        <tr>
                            <td colspan="2">Passport number</td>
                            <td colspan="3">{{$shipping->receiver->passport->passport ?? ''}}</td>
                        </tr>
                        <tr>
                            <td colspan="2">Receiver's Phone number</td>
                            <td colspan="3">
                                @foreach ($shipping->receiver->phones as $phone)
                        ...
                        <tr>
                            ...
                            <td colspan="3">{{ $shipping->number ?? ''}}</td>
                        </tr>
                        <tr>
                    ...
                    <tbody>
                        ...
                        @for ($i = 1; $i < 21; $i++)
                            <tr>
                                <td>{{ $i }}</td>
                                <td>{{$shipping->products[$i-1]->item->name ?? ''}}</td>
                                <td>{{$shipping->products[$i-1]->count ?? ''}}</td>
                                <td>{{$shipping->products[$i-1]->price ?? ''}}</td>
                                <td>{{$shipping->products[$i-1]->total_price ?? ''}}</td>
                            </tr>
                        @endfor
                        <tr>
                            <td colspan="3"></td>
